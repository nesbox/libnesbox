...
{
	return new MemFile(memory);
}

int memgzread(gzFile file, voidp buf, unsigned len)
{
	MemFile* memFile = (MemFile*)file;

	memcpy(buf, memFile->pointer + memFile->position, len);
	memFile->position += len;
...
int memgzwrite(gzFile file, const voidp buf, unsigned len)
{
	...
	memcpy(memFile->pointer + memFile->position, buf, len);
	memFile->position += len;
...
z_off_t memgzseek(gzFile file, z_off_t off, int whence)
{
	MemFile* memFile = (MemFile*)file;
	memFile->position += off;
