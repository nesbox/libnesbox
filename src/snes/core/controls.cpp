...
char *S9xGetCommandName(s9xcommand_t command){
    ...
    switch(command.type){
      case S9xButtonJoypad:
        if(command.button.joypad.buttons==0) return _strdup("None");
        if(command.button.joypad.buttons&0x000f) return _strdup("None");
        s="Joypad";
      ...
      case S9xButtonMouse:
        if(!command.button.mouse.left && !command.button.mouse.right) return _strdup("None");
        s="Mouse";
      ...
      case S9xButtonSuperscope:
        if(!command.button.scope.fire && !command.button.scope.cursor && !command.button.scope.turbo && !command.button.scope.pause && !command.button.scope.aim_offscreen) return _strdup("None");
        s="Superscope";
      ...
      case S9xButtonJustifier:
        if(!command.button.justifier.trigger && !command.button.justifier.start && !command.button.justifier.aim_offscreen) return _strdup("None");
        s="Justifier";
      ...
      case S9xButtonCommand:
        if(command.button.command>=LAST_COMMAND) return _strdup("None");
        return _strdup(command_names[command.button.command]);

      case S9xPointer:
        if(!command.pointer.aim_mouse0 && !command.pointer.aim_mouse1 && !command.pointer.aim_scope && !command.pointer.aim_justifier0 && !command.pointer.aim_justifier1) return _strdup("None");
        s="Pointer";
        c=' ';
      ...
      case S9xButtonPseudopointer:
        if(!command.button.pointer.UD && !command.button.pointer.LR) return _strdup("None");
        if(command.button.pointer.UD==-2 || command.button.pointer.LR==-2) return _strdup("None");
        s="ButtonToPointer ";
      ...
      case S9xButtonMulti:
        {
            if(command.button.multi_idx>=(int)multis.size()) return _strdup("None");
            s="{";
