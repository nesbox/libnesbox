...
uint32 CMemory::RomLoader (uint8 *buffer, int32 maxsize, uint8* source, int sourceSize)
{
	int32	totalSize = 0;
...
uint32 CMemory::LoadROM (uint8* buffer, int length)
{
	int	retry_count = 0;
...
again:
	...
	int32 totalFileSize = RomLoader(ROM, MAX_ROM_SIZE, buffer, length);
