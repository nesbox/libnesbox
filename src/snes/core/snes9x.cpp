...
//    GFX.FrameDisplay=conf.GetBool("Display::DisplayFrameCount", false);
	...
	Settings.SnapshotScreenshots=false;//conf.GetBool("Settings::SnapshotScreenshots", true);
...
size_t buffer_file_write(void* buffer, size_t count, BUFFER_FILE* file)
{
  ...
  memcpy(file->buffer + file->position, buffer, count);
  file->position += count;
  return count;
}
...
char* buffer_file_gets(char* buffer, int count, BUFFER_FILE* file)
{
  if(file->file)
  {
    return fgets(buffer, count, file->file);
  }
  return NULL;
}
...
BUFFER_FILE* buffer_file_open(const char* filename, const char* mode)
{
  ...
  BUFFER_FILE* result = (BUFFER_FILE*)malloc(sizeof(BUFFER_FILE));
