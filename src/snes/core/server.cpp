...
void S9xNPProcessClient (int c)
{
    ...
    switch (header [2] & 0x3f)
    {
		...
		NPServer.ROMName [29] = 0;
            ...
            NPServer.Clients [c].ROMName = _strdup ((char *) &data [4]);
#ifdef NP_DEBUG
...
void S9xNPAcceptClient (int Listen, bool8 block)
{
	...
	if (host)
	{
	    ...
	    NPServer.Clients [i].HostName = _strdup (host->h_name);
	}
        else
        {
            char *ip = inet_ntoa (remote_address.sin_addr);
            if (ip)
                NPServer.Clients [i].HostName = _strdup (ip);
#ifdef NP_DEBUG
